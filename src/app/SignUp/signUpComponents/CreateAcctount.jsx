...
const CreateAcctount = ({ title, loading, setLoading, checkShow }) => {
  ...
  const [checkRole, setCheckRole] = useState("BUYER");
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      useEffect(() => {
        ...
        if (searchRole) {
          setCheckRole(searchRole);
          localStorage.setItem("role", searchRole);
        } else {
          const storedLocalRole = localStorage.getItem("role");
          if (storedLocalRole) {
            setCheckRole(storedLocalRole);
          } else {
            router.push("/");
          }
        }
